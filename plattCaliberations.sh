...
cd /fs/clip-scratch/jyothikv/jvdofs/expDir
mkdir $dnow-PlattCal
temporaryFilePath=/fs/clip-scratch/jyothikv/jvdofs/tempwd
...
do
  ...
  do
    ...
    cd /fs/clip-scratch/jyothikv/jvdofs/expDir/$dnow-PlattCal
    if [ ! -d "$rCategory" ]; then
      mkdir $rCategory
    fi
    cd $rCategory
    ...
    LYRL2004_trainIDs=/fs/clip-ediscovery/jyothi/Research/jvdofs/mfpadat/lyrl2004_tokens_train.dat
    ...
    python /fs/clip-ediscovery/jyothi/Research/jvdofs/Fall2016/scripts/allocation.py false $wd/combined.$rCategory.datafile.temp.dat $wd/pickleFiles/combined.$rCategory.fdat.p $LYRL2004_trainIDs $LYRL2004_b0testIDs $wd/trainfile.dat $wd/testinsf.$testsetSize.dat $testsetSize
    ...
    then
      ...
      plattPrameters=$(python /fs/clip-ediscovery/jyothi/Research/jvdofs/Fall2016/scripts/PlattCaliberation.py $wd/test4Platt.$testsetSize.count.lables.dat $wd/predictions/train/predictions4Platt.$testsetSize.dat 2> $wd/logFiles/PlattCaliberation.$testsetSize.log)
      PARAMETERS="$(echo -e "${plattPrameters}" | tr -d '[[:space:]]')"
      echo "STATUS: Platt caliberation Completed, $PARAMETERS"
      duration=$SECONDS
      echo "$(($duration / 60)) minutes and $(($duration % 60)) seconds elapsed."
      #  echo $plattPrameters
      # python /fs/clip-ediscovery/jyothi/Research/jvdofs/Fall2016/scripts/ProbabilityCaliberation.py $wd $PARAMETERS $wd/predictions/train/predictions4Platt.$testsetSize.dat $wd/trainf.count.lables.dat $trainInsCount
    fi
...
############################################### Saperate above
    ...
    # ./svm_classify $wd/testinsf.$testsetSize.dat $wd/model.trainCount.$testsetSize.dat $wd/predictions/test/prediction.tCount.$testsetSize.dat >> $wd/logFiles/svm.finaltest.$testsetSize.log
    # python /fs/clip-ediscovery/jyothi/Research/jvdofs/Fall2016/scripts/ProbabilityCaliberation.py $wd $PARAMETERS $wd/predictions/test/prediction.tCount.$testsetSize.dat $wd/testinsf.$testsetSize.count.lables.dat $testsetSize
